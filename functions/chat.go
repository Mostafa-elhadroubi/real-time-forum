...
func FetchUsers(w http.ResponseWriter, r *http.Request) {
	...
	query := `SELECT u.user_id, u.username, u.image, m.sent_at AS last_message_time FROM users u LEFT JOIN (SELECT m1.message_id, m1.message, m1.sent_at, CASE 
    WHEN m1.sender_id = ? THEN m1.receiver_id ELSE m1.sender_id END AS other_user_id FROM messages m1 INNER JOIN (
    SELECT MAX(sent_at) AS max_sent_at, CASE WHEN sender_id < receiver_id THEN sender_id ELSE receiver_id END AS user1,
    CASE WHEN sender_id < receiver_id THEN receiver_id ELSE sender_id END AS user2 FROM messages
    WHERE ? IN (sender_id, receiver_id) GROUP BY user1, user2) latest ON ((m1.sender_id = latest.user1 AND m1.receiver_id = latest.user2) OR
    (m1.sender_id = latest.user2 AND m1.receiver_id = latest.user1)) AND m1.sent_at = latest.max_sent_at WHERE ? IN (m1.sender_id, m1.receiver_id)
	) m ON u.user_id = m.other_user_id ORDER BY CASE WHEN m.message_id IS NULL THEN 1 ELSE 0 END ASC, COALESCE(m.sent_at, '1970-01-01') DESC, u.username ASC;`
